...
export const messageRouter = createTRPCRouter({
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // FIX: role must be 'USER' — this is the human's request message.
      // The plan and planStatus fields live on this message because they
      // describe what will be done in response to what the user asked.
      // type 'RESULT' is only for assistant output messages — use 'RESULT'
      // here only because the schema requires it for all messages; the role
      // is what distinguishes user input from assistant output.
      const createdMessage = await prisma.message.create({
        data: {
          projectId: existingProject.id,
          content: input.value,
          role: 'USER',       // FIX: was 'ASSISTANT' — must be 'USER'
          type: 'RESULT',
  ...
  approvePlan: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const message = await prisma.message.findFirst({
        where: {
          id: input.messageId,
          role: 'USER',  // FIX: only USER messages own a plan — guard against
                         // accidentally approving an ASSISTANT result message
          project: {
            OR: [
              { userId: ctx.auth.userId },
              { workspace: { members: { some: { userId: ctx.auth.userId } } } },
      ...
      await inngest.send({
        ...
        data: {
          ...
          messageId: message.id,   // this is now guaranteed to be a USER message ID
  ...
  rejectPlan: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const message = await prisma.message.findFirst({
        where: {
          id: input.messageId,
          role: 'USER',  // only USER messages own a plan
          project: {
            OR: [
              { userId: ctx.auth.userId },
              { workspace: { members: { some: { userId: ctx.auth.userId } } } },
  ...
  // ── Ask: Q&A about the project without generating code ────────────────────
...
${filesContext}`
      ...
      // Save AI reply as ASSISTANT message — no fragment, no credit consumed
  ...
  // ── Suggestions: generate 3 clickable next-step prompts ──────────────────
