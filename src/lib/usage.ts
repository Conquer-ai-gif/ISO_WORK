...
//
//   RULE: logEvent delta MUST always be in units (same as DB balance).
//         Display to user MUST always go through unitsToCredits().
//
// ══════════════════════════════════════════════════════════════════════════════
...
const REFERRAL_BONUS = 5  // credits — multiplied by CREDIT_SCALE before use
 ...
 * e.g. 46 units → 4.6 credits
 * ALL display values must go through this — never use Math.round(balance / CREDIT_SCALE)
 * directly, as that truncates decimals and makes deductions invisible to the user.
 */
...
/**
 * Log a credit event. Delta MUST be in units (not credits) so it stays
 * consistent with the DB balance column. Positive = earned, negative = spent.
 */
...
export async function initCredits(userId: string): Promise<void> {
  ...
  // FIX: log delta in units (50), not credits (5), so event log matches DB balance
  await logEvent(userId, balance, 'signup')
}
...
export async function consumeCreditsV2(opts: ConsumeCreditsOptions): Promise<void> {
  ...
  // Log deduction in units (negative) — consistent with DB balance column
...
export async function consumeCredits(ownerUserId?: string): Promise<void> {
  ...
  // Log in units — costUnits = CREDIT_SCALE = 10 units = 1 credit
  await logEvent(chargeId, -costUnits, 'generation')
}
...
export async function getUsageStatus(userId?: string) {
  ...
  return {
    // FIX: use unitsToCredits() instead of Math.round(balance / CREDIT_SCALE).
    // Math.round truncates decimals — e.g. 46 units would display as 5 instead of 4.6,
    // making deductions invisible to the user after a generation completes.
    balance:   Math.max(0, unitsToCredits(credits.balance)),
    plan:      credits.plan,
...
export async function applyPlanChange(
  ...
  // FIX: log delta in units (newBalance), not credits (PLAN_CREDITS[newPlan]),
  // so event log stays consistent with DB balance column
  await logEvent(userId, newBalance, reason)
}

// ── Daily free-tier top-up ────────────────────────────────────────────────────
// Runs every day at midnight via Inngest cron.
// Tops up any free user whose balance is below 50 units back to 50 units.
// Only users below the max are touched — users who have not spent anything are skipped.

export async function resetFreeCredits(): Promise<number> {
  const now             = new Date()
  const maxBalanceUnits = PLAN_CREDITS.free * CREDIT_SCALE  // 50 units

  // Find users who need a top-up BEFORE updating so we can log events for each
  const usersToTopUp = await prisma.credits.findMany({
    where:  { plan: 'free', balance: { lt: maxBalanceUnits } },
    select: { userId: true, balance: true },
  })

  if (usersToTopUp.length === 0) return 0

  // Atomically top up all qualifying users in one query
  await prisma.credits.updateMany({
    where: { plan: 'free', balance: { lt: maxBalanceUnits } },
    data:  { balance: maxBalanceUnits, lastReset: now },
  })

  // Log a daily_reset event for each user showing exactly how many units were added
  await prisma.creditEvent.createMany({
    data: usersToTopUp.map(u => ({
      userId: u.userId,
      delta:  maxBalanceUnits - u.balance,  // e.g. user had 46 units → delta = 4 units added
      reason: 'daily_reset' as const,
    })),
  })

  return usersToTopUp.length
}
...
export async function applyReferralCode(
  ...
  const bonusUnits = REFERRAL_BONUS * CREDIT_SCALE  // 5 credits × 10 = 50 units
  await prisma.credits.updateMany({
    where: { userId: { in: [referral.ownerId, newUserId] } },
    data:  { balance: { increment: bonusUnits } },
  })

  // FIX: log delta in units (bonusUnits = 50), not credits (REFERRAL_BONUS = 5),
  // so event log stays consistent with DB balance column
  await logEvent(referral.ownerId, bonusUnits, 'referral')
  await logEvent(newUserId,        bonusUnits, 'referral')
