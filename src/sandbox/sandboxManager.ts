...
/**
 * Normalise a file path so it always resolves under /home/user.
 * Handles three cases:
 *   - already absolute: "/home/user/app/page.tsx" → unchanged
 *   - bare relative:    "app/page.tsx"            → "/home/user/app/page.tsx"
 *   - leading slash:    "/app/page.tsx"            → "/home/user/app/page.tsx"
 */
function toSandboxPath(filePath: string): string {
  if (filePath.startsWith(SANDBOX_BASE)) return filePath
  const relative = filePath.replace(/^\/+/, '')
  return `${SANDBOX_BASE}/${relative}`
}
...
export async function restoreFilesIntoSandbox(
  ...
  if (lastFragment?.files) {
    ...
    const description = `Restoring ${entries.length} files from last generation…`
    emit?.(makeEvent('log', { data: { description, message: description } }))

    const sandbox = await getSandbox(sandboxId)
    for (const [path, content] of entries) {
      await sandbox.files.write(toSandboxPath(path), content)
    }
  ...
  try {
    ...
    const description = `Seeding from GitHub — ${project.repoOwner}/${project.repoName}…`
    emit?.(makeEvent('log', { data: { description, message: description } }))
