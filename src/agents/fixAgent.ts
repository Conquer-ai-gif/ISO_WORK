...
async function detectErrors(sandboxId: string, files: string[] = []): Promise<string> {
  try {
    ...
    // Check for missing module errors from the dev server (written by the sandbox runner)
    // next-build-error.log is written by the sandbox template on build failure
...
export async function runFixAgent(options: RunFixAgentOptions): Promise<FixAgentResult> {
  const { sandboxId, existingFiles, failingFiles, allowedFiles, emit, userPlan = 'free', creditsRemaining, maxLoops } = options
  ...
  for (let attempt = 1; attempt <= loopLimit; attempt++) {
    ...
    try {
      const result = await withFallback(
        () => buildFixNetwork(model).run(fixPrompt, { state: fixState }),
        () => buildFixNetwork(freeModel).run(fixPrompt, { state: fixState }),
